<template>
  <div>
    <header class="content-header">
      <h3 class="header-title">{{ $t(`extra.create_campaign`) }}</h3>
      <div class="header-action">
        <!-- <button 
          v-if="hasPermission('extra.campaign.create')"
          class="btn btn-secondary mr-2"
          :class="[addClassCreate()]"
          @click="onClickCreate()">
          <i class="fa-regular fa-download"></i>
        </button> -->
        <div v-if="hasPermission('extra.campaign.create')" class="input-group">
          <input
            v-model="input.name"
            type="text"
            class="form-control"
          ...
          <div class="input-group-append">
            <button type="button" class="btn btn-primary" :disabled="!canCreate" @click="onSubmit">
              สร้างแคมเปญ
            </button>
          </div>
        </div>
      ...
      <div class="row">
        <div v-for="(filter, i) in filterConditions" :key="`filter-${i}`" class="col-12 d-flex mb-3">
          ...
          <select
            ...
            class="form-control mr-3"
            style="min-width: 65px; width: 60px;"
            @change="onUpdateFilter()">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('admin-extra-campaign-one', [
      'records',
      'agents',
      'pagination',
      'responseSuccess',
    ...
    canCreate () {
      return !!this._.size(this.agentIds) && this.input.name
    },
  },
  watch: {
    agentIds: {
    ...
    'responseSuccess': {
      handler (response) {
        if (response.status === 'success') {
          // const id = _.get(response, 'record.id')
          this.$router.push({ path: this.linkTo(`/extra/campaign`) })
        }
      }
    },
    'responseError': {
  ...
  methods: {
    ...mapActions('admin-extra-campaign-one', [
      'getUsers',
      'createCampaign'
    ]),
    /**
    ...
    addClassCreate (index) {
      return this.canCreate ? '' : 'disabled'
    },
    ...
    onSubmit: _.debounce(function () {
      ...
      const filters = {
        ...this.filters,
        agent_ids: this._.split(this.filters.agent_ids, ',')
      }
